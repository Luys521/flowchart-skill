...
| 校验 | 04 结构校验？ | 脚本 | 流程表 | H1–H8 + 表头 H9 | 通过，或逐条硬错误 | 通过→05 ｜ 不通过→**回 03** |
...
- 判据分三层，报错按同一顺序出（先修的排在前面，改完重跑再看下一层）：**① 一行**（编号、语义）→ **② 一种类型**（起止、出边、分支）→ **③ 整张图**（引用、连通、回路）。细则（H1–H8 + 表头 H9）见 `references/flowtable-spec.md` 第 3 节。

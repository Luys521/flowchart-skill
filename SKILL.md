...
**材料根会变**：往目录里补一份合同，就得回 `03` 重跑——`parse.py` 默认就查（`materials[]` 里记的 `root` 与根下当下对不上、或哪份 `sha256` 变了 ⇒ **醒目告警**）；要它**硬拦**就加 `--strict-stale`，单独查用 `probe.py --verify <成果根>/materials.json`（退 2）。

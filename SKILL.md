...
**三份产物的必需性不一样**（别把 ①②③ 读成"都得有"）：**`html` 是必需的**——它的渲染器不在，`build` 直接失败（少一份交付物而没人说，是本项目最忌的假绿）；**`drawio` 与 `svg` 是可选的**——模块不在也照出图，缺了会在**交付清单**里说明（`MISSING_RENDERERS`）。唯一出处是 `scripts/build.py` 的渲染器注册表（`required` 标记）。
...
**AI 不手改表格**——改列会撕表，改行会让 `check` 判不出来。`cells.py fill` 挡四类错：键不在产物里 · 列名不是声明的 AI 列 · 值空 · **待填清单的骨架指纹对不上**（自 D-105 起：清单带着它出生的那版产物的指纹，回比不上就退 1 不写盘）；**任一不合就不写盘**。⚠ 残留的边界只剩一种：**答案 JSON 与待填清单不是同一次 `build` 出的**（指纹绑的是"产物 ↔ 清单"，不绑答案本身）——改过产物就重出清单再填（G67：此前这里写的是"没有指纹能拦"，那是 D-105 之前的旧口径）。
...
**射程**：材料链（`probe` / `parse` / `ledger` / `recon` / `intake` / `plan` / `drift` / `query`）与
`table_to_dsl` / `validate` / `build` / `manifest` / `sync` 都按上表；**`shot.py` 与渲染族例外**——
它们只有 `0` / `1`（缺浏览器、截图失败都报 `1`），别照上表去等一个 `2`（G66：这句原先写的是
"所有脚本同一口径"，与 `shot.py` 的实际行为不符）。

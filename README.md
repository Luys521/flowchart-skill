...
| 质量门禁 | 机器阻断 | 八项几何：引用完整 / 节点不重叠 / 边不穿节点 / 端点接在真实形状边框上 / 边不重叠（正交交叉允许）/ 标签压线 / 画布内 / 网格对齐 |
...
scripts/              渲染与校验脚本（28 模块 = 16 个带 CLI 的入口 + 12 个纯库）
  dictionary.yaml     布局/配色/文字的数值字典
references/           出图时按需读的规范（6 份）
templates/            2 份模板（init.py 拷进 output/<名称>/）
examples/workflow/    **格式基准**（SKILL 把自身工作流画成图，13 节点 / 18 边）
...
| **代码地图自举** | `dev/baseline/self-boot/self-boot-flow.html` | 28 节点 / 27 边，**内嵌 26 张模块子图**（节点 = 该模块的函数，规模现跑现取）、drawio 27 页 |
...
dev/tools/accept.py  十道门全过（0 仪器故障）
```

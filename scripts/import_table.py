# -*- coding: utf-8 -*-
r"""import_table.py — 把**外部节点表**吃成契约流程表（`PIPELINE-SPEC` §4 的落表入口）。

**为什么需要**（2026-09-18 实测）：真实业务的入口常常**已经有一张"准流程表"**
（阶段 / 编号 / 名称 / 类型 / 主体 / 岗位 / 时间 / 下个节点 / 描述），但列数、列名、分支写法都不合契约。
原先只能由 AI **手写**几十行来转（`<br>` 换 `｜`、长标签收进描述、缺列补 `—`、编号与引用逐个核）——
真材料集上那张 38 行的表就是这么手的，而"手写"意味着列数、编号、双向引用全靠人保证。

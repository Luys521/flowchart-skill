...
def _check_node_semantics(id_, name, subj, typ, who, when, errs):
    ...
    _check_obligation_name(id_, name, errs)
    return t
...
# 「义务 / 罚则 / 贯穿性条款」的措辞标记（判据见 `references/material-to-nodes.md` §3）。
# 这些条款**有动作有产出**（发函、赔偿、出证明都是动作），只按"有没有产出"判它们**条条合规**——
# 一份合作框架协议能因此长出五六个"节点"，而它们根本不是流程里的**过程**：它们答不出
# "发生在哪一步之前 / 之后"，是**所有步骤共同的前提**。
# 机器判不了"有没有位置"（那是语义），**但数得出像不像那一类词** ⇒ 只做**软提示**，不阻断（D-55）。
# **只扫节点名称、不扫描述**：把条款写进相邻节点的描述是**正确**做法，扫描述会把对的做法也报出来。
_OBLIGATION_WORDS = ('赔偿', '违约', '违约金', '保密', '不可抗力', '侵权', '争议', '索赔', '罚款')


def _check_obligation_name(id_, name, errs):
    """H7(软)：节点**名称**像"义务 / 罚则 / 贯穿性条款"⇒ 提示回去用 §3 的判据复核。

    ⚠ **它不判对错**——"违约处置"完全可以是真步骤（发函、整改、扣款都有明确位置）。
    它只做一件事：**把可疑的挑出来让你过一遍**。为什么值得有这条提示：这类错误**没有任何硬门禁
    拦得住**（H1–H9 与八项几何对"条款被当成步骤"全绿；实测 2026-09-24：一份框架协议落出 14 节点，
    H1–H9 全绿、几何全过，而其中 5 个按 §3 该砍）——**软提示是这条链上唯一的自动防线**。
    """
    hit = [w for w in _OBLIGATION_WORDS if w in (name or '')]
    if not hit:
        return
    errs.warn(f'H7(软) 节点 {id_}: 名称「{name}」像**条款**而非步骤（命中 {" / ".join(hit)}）'
              f'—— 按 material-to-nodes §3 问一句"它的上一步 / 下一步是什么"，答不上来就该并入描述',
              subject=id_, fix='答不出位置 ⇒ 并入相邻节点的描述，不单独成节点')

...
两张表各司其职：`build` 出的骨架给 AI 填；`check` 只认**收口后**的卡片——它查两件事（§7 登记的那两条），
外加取值封闭与"推断必须留痕"：
...
**不查 element id 存不存在**：那是 **H10.1** 的活（§5 明说"任何产物里出现的每个 element id 都必须在账本里存在"），
...
# - **不查 element id 存不存在**：那是 H10.1（§5）——在这里再写一份 id 提取就是两份真值，必漂。

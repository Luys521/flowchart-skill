...
# - **不猜 .wps/.et/.dps/.xls 这些后缀**：它们原先被写死成"需转换器"，而真样本里合法 xlsx 改名 `.et`
#   是**能直读**的（改名件按内容判）、`.wps` 是 WPS 产出的 Word 97-2003 族。现在一律按 `kind` 说话：
#   真 OLE 才说转换器，合法 xlsx 改名 `.et` 走直读。
#   原先这里还引了"§1.4 明说 et/dps 不支持"——**那是误引**：§1.4 那句话说的是**宿主读工具**
#   （`open_file` / `doc_*` / `slide_*`）的格式缺口，不是本仓的能力边界（§8.2 第 4 条同一类混用）。

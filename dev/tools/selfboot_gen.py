...
# 流水线阶段：把 30 个模块按"它在这个项目里干什么"编成 9 组，写进根表的「项目运作阶段」列。
# 扁平 31 张、不切三层时，**分组信息只落在这里**——不复用这一列，分组在图里就没有据可查。
# 元组顺序即流水线顺序；`_root_rows` 按模块查表，查不到就抛错（仪器失效，不静默留空）。
PIPELINE_STAGES = (
    ('接管与解析', ('probe', 'recon', 'parse_ooxml', 'ledger', 'flowtable', 'semantics', 'artifact')),

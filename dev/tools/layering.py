...
PUBLIC = {
    ...
    'writeback',          # 回写与文件级比对
    'capability',         # 能力指纹（产物是哪一版机制产的）：被账本 / 取子集 / 计划复用；出度 0
...
MODULE = {
    ...
    'import_table',   # → flowtable.md（**可选**：外部节点表 → 契约流程表；节点采集归 AI，§0.1）
...
# `build_all` = 并行出图：一个成果根下所有流程并发 build（§7 的 L4 最小形态，D-126）。
ORCH = {'build', 'sync', 'parse', 'build_all'}

...
- **产物落成果根，默认名 `recon.md`**（登记在 `artifact.NON_TABLE_MD`，否则会被判"孤儿表"，§7.2）。
...
否则一张空卡片也能过校验。校验器只查 §7 登记的那两条（**卡片与账本一致**：材料一一对应、档位逐字相等、
读不动逐字等于 `reason`；**版本关系双向一致**）+ 取值封闭 + "推断必须留痕"；
**不查 element id 存不存在**——那是 **H10.1** 的活（§5），在这里再写一份 id 提取就是两份真值。

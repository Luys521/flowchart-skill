...
| **stderr 编码**：往 stderr 打非 ASCII 的模块 16 个，其中 **14 配了编码、2 个没配**（`drift` / `thresholds`） | 实测 `drift` 的错误分支：stderr 原始字节按 utf-8 解出来是 `\u26a0 ��������ˣ…`（`⚠` 不在 GBK 里 ⇒ 退化） | **已修 + 已上仪器**（D-128）：`drift.py` 补一行（它是 CLI）；`thresholds` 是**纯库**、豁免（它的 6 个调用方全是 CLI）。面③ 加一条：带 CLI 的模块若往 stderr 打非 ASCII，必须自己配编码——**豁免名单打出来**，不让这份依赖隐形 |

**唯一一条留作待办的**：`manifest._default_shapes()` 读不动/缺段时**静默退回 `{}`**（`scripts/manifest.py:120`）。
它与"降级必须留痕"冲突，而且症状是**下游报一堆形状不符**（把"字典缺段"说成"产物形状错"）。
修法是留痕（一行说明"`shapes` 段读不出来，形状反查退化为空"），但它落在**公共层纯库**、
又要往 stderr 写中文 ⇒ 要一起定"库怎么留痕"。**没有当场改的理由不是"小"，是它牵到上面那条 D-128 的边界。**

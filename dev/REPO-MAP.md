...
**31 个模块 = 19 个带 CLI 的入口 + 12 个纯库。**

| 类型 | 模块 |
|---|---|
| 入口命令（有 `__main__`） | `init` `table_to_dsl` `build` `validate` `shot` `sync` `xml_reader` `clarify` `layer_index` `manifest` `render_html` `render_drawio` `render_svg` `writeback` `probe` `recon` `parse_ooxml` `parse_pdf` `ledger` |
...
| **模块层**（14） | `init` `clarify` `table_to_dsl` `layer_index` `render_html` `render_drawio` `render_svg` `validate` `shot` `probe` `recon` `parse_ooxml` `parse_pdf` `ledger` | 各有产物；**只许依赖公共层**；彼此之间**没有代码依赖**——协作走产物 |
| **编排层**（2） | `build` `sync` | 流水线驱动者，允许依赖上面两层 |

实测（31 模块 / 78 条依赖边）：`module→public` 31 条 · `orch→module` 8 条 · `orch→orch` 1 条 ·

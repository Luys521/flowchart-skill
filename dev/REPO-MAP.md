...
| 层 | 模块 | 规则 |
|---|---|---|
| **公共层**（19） | `semantics` `geometry` `artifact` `cells` `capability` `textquality` `pptx_text` `flowtable_layout` `flowtable` `flowtable_check` `flowtable_colors` `router` `lane_router` `swimlane` `label` `engine` `manifest` `xml_reader` `writeback` | 被多方复用的纯能力；**可以互相引用**，但**不许依赖上层** |
...
实测（44 模块 / 95 条依赖边）：`module→public` 46 条 · `orch→module` 9 条 · `orch→orch` 1 条 ·

...
**36 个模块 = 23 个带 CLI 的入口 + 13 个纯库。**

| 类型 | 模块 |
|---|---|
| 入口命令（有 `__main__`） | `init` `table_to_dsl` `build` `validate` `shot` `sync` `xml_reader` `clarify` `layer_index` `manifest` `render_html` `render_drawio` `render_svg` `writeback` `probe` `parse` `recon` `parse_ooxml` `parse_pdf` `parse_legacy` `parse_text` `intake` `ledger` |
...
| **模块层**（17） | `init` `clarify` `table_to_dsl` `layer_index` `render_html` `render_drawio` `render_svg` `validate` `shot` `probe` `recon` `parse_ooxml` `parse_pdf` `parse_legacy` `parse_text` `intake` `ledger` | 各有产物；**只许依赖公共层**；彼此之间**没有代码依赖**——协作走产物 |
| **编排层**（3） | `build` `sync` `parse` | 流水线驱动者，允许依赖上面两层（`parse` 按固定顺序跑各解析适配器，判据不在它那里） |

实测（36 模块 / 79 条依赖边）：`module→public` 31 条 · `orch→module` 8 条 · `orch→orch` 1 条 ·

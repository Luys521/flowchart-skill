...
**`build.py` 内部六环**（全部脚本自动，AI 不介入）：节点类型分析 → 关系分析 → 空间规划 → 节点连接 → 碰撞检测 → 产物审核。前两环读《流程表》、第三环读 DSL，**都不看产物**，所以第六环拿契约反查产物把最后一环焊上。
...
**40 个模块 = 26 个带 CLI 的入口 + 14 个纯库。**

| 类型 | 模块 |
|---|---|
| 入口命令（有 `__main__`） | `init` `table_to_dsl` `build` `validate` `shot` `sync` `xml_reader` `clarify` `layer_index` `manifest` `render_html` `render_drawio` `render_svg` `writeback` `probe` `parse` `recon` `parse_ooxml` `parse_pdf` `parse_legacy` `parse_text` `render_pages` `intake` `ledger` `drift` `query` |
| 纯库（无 CLI，只被 import） | `artifact` `engine` `flowtable` `flowtable_check` `flowtable_colors` `flowtable_layout` `geometry` `label` `lane_router` `pptx_text` `router` `semantics` `swimlane` `textquality` |

**分层（2026-09-14 定，由 `dev/tools/layering.py` 守住）**：

| 层 | 模块 | 规则 |
|---|---|---|
| **公共层**（17） | `semantics` `geometry` `artifact` `textquality` `pptx_text` `flowtable_layout` `flowtable` `flowtable_check` `flowtable_colors` `router` `lane_router` `swimlane` `label` `engine` `manifest` `xml_reader` `writeback` | 被多方复用的纯能力；**可以互相引用**，但**不许依赖上层** |
| **模块层**（20） | `init` `clarify` `table_to_dsl` `layer_index` `render_html` `render_drawio` `render_svg` `validate` `shot` `probe` `recon` `parse_ooxml` `parse_pdf` `parse_legacy` `parse_text` `render_pages` `intake` `ledger` `drift` `query` | 各有产物；**只许依赖公共层**；彼此之间**没有代码依赖**——协作走产物 |
| **编排层**（3） | `build` `sync` `parse` | 流水线驱动者，允许依赖上面两层（`parse` 按固定顺序跑各解析适配器，判据不在它那里） |

实测（40 模块 / 84 条依赖边）：`module→public` 36 条 · `orch→module` 8 条 · `orch→orch` 1 条 ·
`orch→public` 12 条 · `public→public` 27 条 · **违规 0 条**（数字随代码增长，现跑现取：`python dev/tools/layering.py`）。

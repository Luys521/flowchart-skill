...
**32 个模块 = 20 个带 CLI 的入口 + 12 个纯库。**

| 类型 | 模块 |
|---|---|
| 入口命令（有 `__main__`） | `init` `table_to_dsl` `build` `validate` `shot` `sync` `xml_reader` `clarify` `layer_index` `manifest` `render_html` `render_drawio` `render_svg` `writeback` `probe` `recon` `parse_ooxml` `parse_pdf` `parse_legacy` `ledger` |
...
实测（32 模块 / 78 条依赖边）：`module→public` 31 条 · `orch→module` 8 条 · `orch→orch` 1 条 ·

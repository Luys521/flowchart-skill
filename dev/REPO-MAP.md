...
| 纯库（无 CLI，只被 import） | `artifact` `engine` `flowtable` `flowtable_check` `flowtable_colors` `flowtable_layout` `geometry` `label` `lane_router` `router` `semantics` `swimlane` `textquality` |

**分层（2026-09-14 定，由 `dev/tools/layering.py` 守住）**：

| 层 | 模块 | 规则 |
|---|---|---|
| **公共层**（16） | `semantics` `geometry` `artifact` `textquality` `flowtable_layout` `flowtable` `flowtable_check` `flowtable_colors` `router` `lane_router` `swimlane` `label` `engine` `manifest` `xml_reader` `writeback` | 被多方复用的纯能力；**可以互相引用**，但**不许依赖上层** |
...
实测（39 模块 / 82 条依赖边）：`module→public` 34 条 · `orch→module` 8 条 · `orch→orch` 1 条 ·

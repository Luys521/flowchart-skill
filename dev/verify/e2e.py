...
| 受理 | 02 | 资料是否齐全？ | 判断 | 申请材料 | 齐全清单 | 齐全 / 不齐全 | 甲方 | 受理员 | — | 齐全→03 ｜ 材料只读、永不修改→回 01 补件 | 齐全才受理，否则退回补件★ |
...
def run_face(tmp):
    ...
    d = prod(e2e, 'drawio')
    # 这张夹具的标签故意放长到**必然折排**（`材料只读、永不修改` 徽章 124 > 上限 80），而且折点落在
    # **全角顿号**上——正是 G89 那个"按相邻字符猜并法"会猜错的位置。夹具不这么放的话，折排这条路
    # 在产物侧零实例，放宽断言买不到任何覆盖面（审查实测：原夹具 `不齐全` 徽章 52 < 派生档距 60 ⇒ 不折）。
    s = (d.read_text(encoding='utf-8').replace('&gt;归档&lt;', '&gt;资料归档&lt;')
         .replace('value="材料只读、&lt;br&gt;永不修改"', 'value="否"')
         .replace('value="材料只读、永不修改"', 'value="否"'))
    ...
    c.check('资料归档' in _plus and '材料只读、永不修改' in _minus,

...
| 模块级 | 06 | load_thresholds | 任务 | — | — | — | 脚本 | selfboot | — | →11 | ★ 阈值 = 内置默认 + `dictionary.yaml` 的 `pdf_text_layer:` 段（读不到就用默认… · L78 · 函数 |
| 模块级 | 07 | thin_note | 任务 | — | — | — | 脚本 | selfboot | — | →11 | ★ **尺子二**（§1.6）：`"抽出来太少"` → 一句降级说明；不判薄返回空串。 · L94 · 函数 |
| 模块级 | 08 | parse_pdf | 任务 | — | — | — | 脚本 | selfboot | — | 1→02｜2→06｜3→07 | ★ `.pdf` → `(elements, 采样说明, 报错文案)`。只抽文本层；空页跳过（不伪造 element）。 · L115 · 函数 · 分支：1→_import_dep 2→load_thresholds 3→thin_note |
| 模块级 | 09 | parse_materials | 任务 | — | — | — | 脚本 | selfboot | — | →08 | ★ 材料层 → `(elements, 补注, 摘要, 跳过清单, 报错文案)`。非 PDF / 非 T2 一律跳过并记账。 · L168 · 函数 |
| 模块级 | 10 | main | 任务 | — | — | — | 脚本 | selfboot | — | 1→03｜2→04｜3→05｜4→09 | L218 · 函数 · 分支：1→_read_json 2→_write_notes 3→_page_span 4→parse_materials |

...
| 模块级 | 02 | parse_range | 任务 | — | — | — | 脚本 | selfboot | — | →11 | ★ `'pages=40-60'` → `('pages', 40, 60)`；`'sheet=清单'` → `('she… · L65 · 函数 |
| 模块级 | 03 | in_range | 任务 | — | — | — | 脚本 | selfboot | — | →11 | ★ 这条落在任一范围里吗（`ranges` 为空 = 不筛）。`ordinal` 是该材料内第几条（1 起）。 · L88 · 函数 |
| 模块级 | 04 | matches | 任务 | — | — | — | 脚本 | selfboot | — | →11 | ★ 材料号与关键词两关（都为空 = 不筛）。可搜面来自公共层（与 `parse.py --grep` 同一句）。 · L103 · 函数 · ⇢ 依赖 textquality.element_haystack |
| 模块级 | 05 | loc_text | 任务 | — | — | — | 脚本 | selfboot | — | →11 | ★ `location` → 一句人能读的位置（页 / 子表!单元格 / 图内坐标 / 实在没有就报"没有出处"）。 · L110 · 函数 |

...
| 模块级 | 02 | parse_range | 任务 | — | — | — | 脚本 | selfboot | — | →11 | ★ `'pages=40-60'` → `('pages', 40, 60)`；`'sheet=清单'` → `('she… · L66 · 函数 |
...
| 模块级 | 09 | load_defaults | 任务 | — | — | — | 脚本 | selfboot | — | →11 | ★ 默认值 = 内置 + `dictionary.yaml` 的 `query:` 段（读不到就用内置，**不报错**）。 · L173 · 函数 |
| 模块级 | 10 | main | 任务 | — | — | — | 脚本 | selfboot | — | 1→02｜2→05｜3→06｜4→07｜5→08｜6→09 | ★ 命令行入口：读账本 → 解析范围 → 取一批 → 打印（或 `--json`）。 · L189 · 函数 · 分支：1→parse_range 2→loc_text 3→excerpt 4→select 5→render 6→load_defaults · ⇢ 依赖 capability.compare、capability.read_json、capability.warn |

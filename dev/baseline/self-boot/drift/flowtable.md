...
| 模块级 | 02 | _col | 任务 | — | — | — | 脚本 | selfboot | — | →29 | ★ 按**列名前缀**取值（`状态（AI 填）` / `走哪条路` 这类名字前缀足够稳，后缀由对方决定）。 · L115 · 函数 |
| 模块级 | 03 | _ids | 任务 | — | — | — | 脚本 | selfboot | — | →29 | ★ 单元格 → 引用到的 id 集合（element id 与裸 `M##` 都在内）。 · L123 · 函数 |
| 模块级 | 04 | _mats | 任务 | — | — | — | 脚本 | selfboot | — | →03 | ★ 单元格 → 引用到的**材料号**集合（`M03#p012` → `M03`）。 · L128 · 函数 |
| 模块级 | 05 | _cited | 任务 | — | — | — | 脚本 | selfboot | — | →03 | ★ 全表引用到的 id 集合：依据列 + 节点描述（描述里也会写"见 M05"这类引用）。 · L133 · 函数 |
| 模块级 | 06 | rule_d1 | 任务 | — | — | — | 脚本 | selfboot | — | →03 | ★ D1 等级拔高：依据指向 `certainty=inferred`（`extractor=vlm`）或带 `degra… · L142 · 函数 · ⇢ 依赖 semantics.pending_kind |
| 模块级 | 07 | rule_d2 | 任务 | — | — | — | 脚本 | selfboot | — | 1→02｜2→04 | ★ D2 假设被推翻仍在用：假设账里 `状态 = 已推翻` 的材料，表里还有节点依据它。 · L176 · 函数 · 分支：1→_col 2→_mats |
| 模块级 | 08 | rule_d3 | 任务 | — | — | — | 脚本 | selfboot | — | →04 | ★ D3 建立在读不动的材料上：依据里的材料在账本里 `status=unreadable`——断言的来源不存在。 · L187 · 函数 |
| 模块级 | 09 | rule_d4 | 任务 | — | — | — | 脚本 | selfboot | — | 1→02｜2→05 | ★ D4 含流程的材料零引用 → **缺口**：这份材料的信息没进表（漏读，或读了没落表）。 · L200 · 函数 · 分支：1→_col 2→_cited |

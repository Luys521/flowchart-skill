...
| 模块级 | 02 | _read_json | 任务 | — | — | — | 脚本 | selfboot | — | →11 | ★ 读 JSON（容忍 BOM — PowerShell 的 `Out-File -Encoding utf8` 会写 B… · L46 · 函数 |
| 模块级 | 03 | check_materials | 任务 | — | — | — | 脚本 | selfboot | — | →11 | ★ 材料层校验 → 错误清单（空 = 过）。逐条说清**哪里错了**，不返回布尔。 · L51 · 函数 |
| 模块级 | 04 | check_elements | 任务 | — | — | — | 脚本 | selfboot | — | →11 | ★ 证据层校验 → 错误清单。`kind` 是**封闭枚举**（§2.1），扩枚举要先改规范。 · L77 · 函数 |

...
| 模块级 | 15 | _parse_nodes | 任务 | — | — | — | 脚本 | selfboot | — | 1→13｜2→14 | ★ 解析节点：先 <object>（自有格式），再 <mxCell vertex=1>（外部标准格式） · L218 · 函数 · 分支：1→_native_nodes 2→_external_nodes |
| 模块级 | 16 | _lanes_bbox | 任务 | — | — | — | 脚本 | selfboot | — | →06 | ★ 泳道底色的外包盒（所有带 `flowchartSkillBg=1` 的单元格），没有则 None。 · L225 · 函数 |
| 模块级 | 17 | _band_width | 任务 | — | — | — | 脚本 | selfboot | — | →06 | ★ 左侧里程碑带宽度 = 所有 `x=0` 的 vertex 单元格里最宽的那个。 · L241 · 函数 |
| 模块级 | 18 | _parse_edges | 任务 | — | — | — | 脚本 | selfboot | — | 1→04｜2→09｜3→10 | L257 · 函数 · 分支：1→_label_text 2→_frac 3→_num |
| 模块级 | 19 | _finalize_types | 任务 | — | — | — | 脚本 | selfboot | — | →42 | ★ 端点符节点定起止（胶囊或椭圆，见 `_style_type`）： · L286 · 函数 |
| 模块级 | 20 | _cluster | 任务 | — | — | — | 脚本 | selfboot | — | →42 | ★ 把一维坐标聚成层：返回 {索引: 层号}（按坐标升序） · L311 · 函数 |
| 模块级 | 21 | assign_grid | 任务 | — | — | — | 脚本 | selfboot | — | 1→11｜2→20 | ★ 按中心坐标聚类推断 row/col 与列中心 col_x（容差：行 0.6×高中位数、列 0.9×宽中位数）。 · L323 · 函数 · 分支：1→_median 2→_cluster · ⇢ 依赖 geometry.snap |

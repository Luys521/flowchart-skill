...
| 模块级 | 08 | _quality_gate | 任务 | — | — | — | 脚本 | selfboot | — | →12 | ★ 跑质量门禁；未过则报错并返回 1。 · L150 · 函数 · ⇢ 依赖 validate.main |
| 模块级 | 09 | _source_guard | 任务 | — | — | — | 脚本 | selfboot | — | →12 | ★ 《流程表》自上次渲染后是否被直改过 → `(state, 说明)`。 · L161 · 函数 · ⇢ 依赖 artifact.artifact_stem、manifest.manifest_path_for、manifest.source_stale |
| 模块级 | 10 | _apply | 任务 | — | — | — | 脚本 | selfboot | — | →12 | ★ --apply：两道覆盖门禁通过后覆盖原表/DSL 并重渲染，返回退出码。 · L173 · 函数 · ⇢ 依赖 artifact.artifact_stem、build.main、flowtable.parse_table、writeback.branch_conflicts、writeback.format_conflicts |
| 模块级 | 11 | main | 任务 | — | — | — | 脚本 | selfboot | — | 1→03｜2→04｜3→05｜4→06｜5→07｜6→08｜7→09｜8→10 | L227 · 函数 · 分支：1→_parse_args 2→_resolve_paths 3→_read_back 4→_write_back 5→_gen_dsl 6→_quality_gate 7→_source_guard 8→_apply |
